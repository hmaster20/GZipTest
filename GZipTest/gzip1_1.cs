...
namespace GZipTest
{
    class gzip1_1
    {
        ...
        // О...это те ещё грабли! Если файл размером 1900 байт, потоков 8, а размер буфера под каждый поток - 1000 байт(например), то создаётся всего 2 потока.
        ...
        //Ещё есть фича по поводу размера блока - размер блока, как и количество потоков, выбирается автоматически, в зависимости от машины на которой запущена программа.
